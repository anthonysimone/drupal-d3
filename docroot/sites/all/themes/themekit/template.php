...
include_once './' . drupal_get_path('theme', 'themekit') . '/includes/themekit_assets.inc';
...
function themekit_preprocess_page(&$variables, $hook) {
  drupal_add_js(drupal_get_path('theme', 'themekit') .'/js/utils.js', array(
      'scope' => 'footer'
  ));
}
// */
...
function themekit_preprocess_html(&$variables) {
  ...
  if (isset($html5_respond_meta['selectivizr']) && $html5_respond_meta['selectivizr']) {
    drupal_add_js(drupal_get_path('theme', 'themekit') . '/js/selectivizr-min.js', array(
      'group' => JS_THEME,
    ));
  }

  // Set js and css paths.
  $js_path = drupal_get_path('theme', 'themekit') . '/js/';
  $css_path = drupal_get_path('theme', 'themekit') . '/css/';

  themekit_add_assets($js_path, $css_path, array(
    'js_filename' => 'theme.js',
    'js_filename_min' => 'theme.min.js',
 ...
 */
function themekit_preprocess_menu_block_wrapper(&$variables) {
 ...
 * the <ul> later on.
 */
function themekit_menu_tree__menu_block($variables) {
  return $variables['tree'];
}

/**
 * Override of theme_menu_link()
 * Due to the overrides in themekit_menu_tree__menu_block() we 
 ...
 *   of nested menus.
 */
function themekit_menu_link__menu_block($variables) {
